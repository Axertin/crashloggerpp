...
static void writeMinidump(EXCEPTION_POINTERS *exInfo)
{
    ...
    g_MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile, CLPP_MiniDumpWithThreadInfo, &mei, nullptr, nullptr);
...
static LONG CALLBACK crashHandler(EXCEPTION_POINTERS *exInfo)
{
    ...
    offset = crashAppend(g_CrashBuffer, bufSize, offset, "=== CRASHLOGGER++ CRASH REPORT ===\n");
...
void install(const wchar_t *crashDirectory, bool enableMinidump)
{
    ...
    std::filesystem::create_directories(g_CrashDirPath);
